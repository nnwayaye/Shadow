'use client';
          ...
          <section style={{ paddingTop: 20 }}>
            {synopsisParagraphs.length > 0 ? (
              synopsisParagraphs.map((paragraph, index) => (
                <p key={index} style={summaryParagraphStyle}>{paragraph}</p>
              ))
            ) : (
              <p style={{ color: "var(--muted)", margin: 0 }}>အကျဉ်းချုပ် မရှိသေးပါ။</p>
            )}
          </section>
        ) : (
          <section style={{ paddingTop: 20 }}>
            {pageCount > 1 && (
              <div style={paginationStyle} aria-label="အခန်းစာရင်း စာမျက်နှာများ">
                <button type="button" onClick={() => changePage(currentPage - 1)} disabled={currentPage === 1} style={pageButtonStyle(currentPage === 1)}>‹</button>
                {Array.from({ length: pageCount }, (_, index) => index + 1).map((page) => (
                  <button type="button" key={page} onClick={() => changePage(page)} style={pageNumberStyle(page === currentPage)}>{page}</button>
                ))}
                <button type="button" onClick={() => changePage(currentPage + 1)} disabled={currentPage === pageCount} style={pageButtonStyle(currentPage === pageCount)}>›</button>
              </div>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {visibleChapters.length > 0 ? (
                visibleChapters.map((chapter, index) => (
                  <a key={chapter.id} href={"/book/" + novelId + "/" + chapter.chapter_number} style={chapterLinkStyle}>
    ...
    const summaryParagraphStyle = { margin: "0 0 22px", textIndent: "1.5em", lineHeight: 2.05, whiteSpace: "pre-wrap" };
    ...
    const chapterLinkStyle = { display: "flex", alignItems: "center", gap: 14, padding: "13px 16px", background: "var(--card-surface)", border: "1px solid var(--card-border)", borderRadius: 8, color: "var(--text)", textDecoration: "none" };
    const chapterIndexStyle = { minWidth: 24, color: "var(--muted)", fontWeight: 700 };
